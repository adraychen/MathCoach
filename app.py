...
# ---------------------------------------------------
# OPTIONAL DATABASE / GENERATION MODES
# ---------------------------------------------------
# These imports are optional so the original Practice and Extract Questions
# modes still run even before the new files are placed in the repo.
try:
    from generators.question_generation_ui import run_question_generation_mode
except Exception as import_error:
    run_question_generation_mode = None
    QUESTION_GENERATION_IMPORT_ERROR = import_error
else:
    QUESTION_GENERATION_IMPORT_ERROR = None

try:
    from extraction.past_paper_db_ui import run_past_paper_extraction_db_mode
except Exception as import_error:
    run_past_paper_extraction_db_mode = None
    PAST_PAPER_DB_IMPORT_ERROR = import_error
else:
    PAST_PAPER_DB_IMPORT_ERROR = None
...
MODE_OPTIONS = {
    ...
    "AI Question Generation Test": "generate",
    "Past Paper Extraction to Database": "past_paper_db",
}
...
with st.sidebar:
    ...
    current_label = next(
        (label for label, value in MODE_OPTIONS.items() if value == st.session_state.app_mode),
        "Practice",
    )
...
# ===================================================
# AI QUESTION GENERATION TEST MODE
# ===================================================

elif st.session_state.app_mode == "generate":

    if run_question_generation_mode is None:
        st.title("AI Question Generation Test")
        st.error("Question generation UI module could not be imported.")
        st.info("Place `mathcoach_streamlit_question_generation_ui.py` at `generators/question_generation_ui.py` and `mathcoach_generate_visual_data_questions_groq.py` at `generators/mathcoach_generate_visual_data_questions_groq.py`.")
        st.exception(QUESTION_GENERATION_IMPORT_ERROR)
    else:
        run_question_generation_mode()

# ===================================================
# PAST PAPER EXTRACTION TO DATABASE MODE
# ===================================================

elif st.session_state.app_mode == "past_paper_db":

    if run_past_paper_extraction_db_mode is None:
        st.title("Past Paper Extraction to Database")
        st.error("Past-paper database extraction UI module could not be imported.")
        st.info("Place `mathcoach_streamlit_past_paper_db_ui.py` at `extraction/past_paper_db_ui.py`.")
        st.exception(PAST_PAPER_DB_IMPORT_ERROR)
    else:
        run_past_paper_extraction_db_mode()
